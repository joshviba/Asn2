...
SET ECHO ON
SPOOL "C:\Users\Paul\Desktop\School Stuff\COMP 2714\Asn\2.txt"
...
-- Q2 --
SELECT hotelName, hotelAddress, Room.roomNo, dateFrom, dateTo
  FROM Hotel
  INNER JOIN Room
    ON Hotel.hotelNo = Room.hotelNo
  INNER JOIN Booking
...
-- Q8 --
SELECT hotelName, type, SUM(price) AS "Total Revenue"
  FROM Hotel
  INNER JOIN Room
    ON Room.hotelNo = Hotel.hotelNo
  INNER JOIN Booking
    ON Booking.roomNo = Room.roomNo
  WHERE dateFrom <= DATE'2017-01-29'
    AND (dateTo IS NULL OR dateTo >= Date'2017-01-29')
...
-- Q10 --
SELECT COUNT(hotelName) AS totalHotels
     , COUNT(
          SELECT hotelName
            FROM Hotel
            FULL JOIN Room
